...
class TestAsyncVideoImport:
    """Tests for async video import endpoints."""
    ...
    def test_tiktok_import_returns_202(
        ...
        monkeypatch.setattr(tiktok_scraper, "get_tiktok_context", mock_get_tiktok_context)
        monkeypatch.setattr(OpenAIService, "get_response", mock_get_response)
    ...
    def test_youtube_import_without_openai_returns_400(
        self,
        api_client: TestClient,
        monkeypatch: pytest.MonkeyPatch,
        unique_user: TestUser,
    ):
        """Test that YouTube import fails when OpenAI is not enabled."""
        # Mock settings to disable OpenAI
        ...
        monkeypatch.setattr(settings, "OPENAI_ENABLED", original_enabled)
    ...
    def test_youtube_import_with_language_options(
        ...
        async def mock_get_response(self, prompt: str, message: str, *args, **kwargs) -> OpenAIRecipe | None:
            return OpenAIRecipe(
                ...
                ingredients=[OpenAIRecipeIngredient(text="ingrediente")],
